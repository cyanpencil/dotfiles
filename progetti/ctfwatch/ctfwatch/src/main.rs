...
fn extra_bytes (s1: &str) -> usize {
    s1.len() - strip_ansi_escapes::strip(&s1).unwrap().len()
}

fn double_line (s1: &str, s2: &str) -> String {
...
fn justdoit() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for event in v {
        ...
        text.push(double_line(&format!("{}: {}", blue("website"), gray(event.url.unwrap_or_else(|| "None".to_owned()))),
                              &format!("")));
...
fn http_headers_end(buffer: &str) -> bool {
    if buffer.is_empty() {return false;}
    if buffer.chars().rev().take(4).collect::<String>() == "\n\r\n\r" {
        return true;
    }
    false
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for stream in listener.incoming() {
        ...
        let mut bstream = BufReader::new(&stream);
        while !http_headers_end(&buffer) {
            bstream.read_line(&mut buffer)?;
        }
