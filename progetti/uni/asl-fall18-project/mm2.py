    ...
    if test['from'] == 'middleware' and "_" not in client: #its a middleware
        ...
        for log in os.listdir(os.path.join(basefolder, client)):
            ...
            if rep not in data[req_type][no_clients][work]:
                ...
                threads[no_clients][work][rep] = dict((k, {}) for k in keywords2)
            ...
            with open(os.path.join(os.path.join(basefolder, client, log)), "r") as mylog:
                for line in mylog.readlines():
                    ...
                    thread = int(d['thread']) + (work * (len(clients) - 1))
                    for k in keywords2:
                        if thread not in threads[no_clients][work][rep][k]:
                            threads[no_clients][work][rep][k][thread] = []
                        if k == 'throughput':
                            data[req_type][no_clients][work][rep]['throughput'] += [(float(d['gets']) + float(d['sets']))/test_time]
                            threads[no_clients][work][rep]['throughput'][thread] += [(float(d['gets']) + float(d['sets']))/test_time]
                        else:
                            data[req_type][no_clients][work][rep][k] += [float(d[k])]
                            threads[no_clients][work][rep][k][thread] += [float(d[k])]
        ...
        for work in sorted(workers):
            # for a in range(3):
                # for t in range(work*2):
                    # mu_dict[work] = max(mu_dict[work], np.sum(threads[no_clients][work][a+1]['throughput'][t]))
            for t in range(work*2):
                ttt = 0
                for a in range(3):
                    ttt += np.sum(threads[no_clients][work][a+1]['throughput'][t])
                mu_dict[work] = max(mu_dict[work], ttt/3.0)
...
for work in sorted(workers):
    ...
    for n in range(len(y['set'][work]['resp_time'])):
        total = 0
        for i in range(m):
            total += ((m*ro[n])**i)/fact(i)

        pzero = 1.0/(1 + ((m*ro[n])**m/(fact(m)*(1.0 - ro[n]))) + total)
        q = pzero*((m*ro[n])**m) / (fact(m)*(1.0 - ro[n]))
        print("%10.2f" % ((1 + (q / (m*(1.0 - ro[n])))) / mu), end=' ')
    print()
    print()

    print("%20s" % "Queue size", end=' ')
    for n in range(len(y['set'][work]['queue_size'])):
        print("%10.2f" % float(y['set'][work]['queue_size'][n]*2), end=' ')
    print()
    print("%20s" % "$E[n_q]$", end=' ')
    for n in range(len(y['set'][work]['resp_time'])):
        total = 0
        ...
        pzero = 1.0/(1 + (((m*ro[n])**m)/(fact(m)*(1.0 - ro[n]))) + total)
        q = pzero*((m*ro[n])**m) / (fact(m)*(1.0 - ro[n]))
        print("%10.2f" % (ro[n]*q/(1.0 - ro[n])), end=' ')
    ...
    for n in range(len(y['set'][work]['queue_time'])):
        ...
        pzero = 1.0/(1 + ((m*ro[n])**m/(fact(m)*(1.0 - ro[n]))) + total)
        q = pzero*((m*ro[n])**m) / (fact(m)*(1.0 - ro[n]))
        print("%10.2f" % (q/ (m*mu*(1.0 - ro[n]))), end=' ')
    print()
    print()
