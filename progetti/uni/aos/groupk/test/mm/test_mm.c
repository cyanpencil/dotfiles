...
void alloc_free(struct mm* mm) { const uint32_t NODE_NO      = 2;
    ...
    uint32_t old_free_nodes_no  = count_alloc(dlist_head(&mm->free_buckets[bucket_type(NODE_SIZE)]), NodeType_Free);
    ...
    uint32_t new_free_nodes_no = count_alloc(dlist_head(&mm->free_buckets[bucket_type(NODE_SIZE)]), NodeType_Free);
    /*debug_printf("POST\n");*/
    /*print_list(mm);*/
...
void aligned_test_256 (struct mm * mm) {
    ...
    uint32_t old_free_nodes_no  = count_alloc_size(dlist_head(&mm->free_buckets[bucket_type(NODE_SIZE)]), NodeType_Free, page_round_up(NODE_SIZE));
    for (int i = 0; i < NODE_NO; i++) {
        struct frame_identity fi;
        frame_identify(cap[i], &fi);
        mm_free (mm, cap[i], fi.base, fi.bytes);
    } 
    uint32_t new_free_nodes_no = count_alloc_size(dlist_head(&mm->free_buckets[bucket_type(NODE_SIZE)]), NodeType_Free, page_round_up(NODE_SIZE));
    assert(new_free_nodes_no - old_free_nodes_no == NODE_NO);
}
...
void dlinked_list_test(struct mm* mm) {
    ...
    dlist_remove(&list, (dlinked_node_t*)node_a);
    node = (test_node*) dlist_find(&list, &_find_node_A);
    assert(node == NULL);

    dlist_remove(&list, (dlinked_node_t*)node_c);
    node = (test_node*) dlist_find(&list, &_find_node_C);
    assert(node == NULL);

    dlist_remove(&list, (dlinked_node_t*)node_b);
