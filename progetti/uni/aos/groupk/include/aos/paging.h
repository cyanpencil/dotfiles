...
#define INVALID_OFFSET (void*) 0xffffffff
...
struct pgmapping {
    struct pgmapping *prev;
    struct pgmapping *next;
    lvaddr_t base;       
    lvaddr_t end;        
    struct capref mapping_cap;
    struct capref table_cap;
    struct pgmapping *linked_next;
};
...
struct paging_state {
    ...
    struct slab_allocator pgchunk_alloc;
    dlinked_list_t chunklist;
    dlinked_list_t chunk_reserved_list;

    struct slab_allocator pgmapping_alloc;
    dlinked_list_t pgmapping_list;

    struct capref l1page_cap;

    mapping_cb_t mapping_cb;
    void* si;

    struct thread_mutex mutex, frame_mutex;
};

struct __attribute__((packed, aligned(4))) paging_state_serial {
    uint32_t magic;

    bool l2_mapped[ARM_L1_MAX_ENTRIES];
    struct cnoderef bck_cnode;
    cslot_t bck_cnode_l2_start;
    cslot_t bck_cnode_mp_start;
...
struct __attribute__((packed, aligned(4))) cl_serial {
    lvaddr_t vaddr;
    size_t bytes;
};

struct __attribute__((packed, aligned(4))) pm_serial {
    lvaddr_t vaddr;
    size_t bytes;
};
...
errval_t paging_map_serialize(struct paging_state *st, struct paging_state_serial *sst, size_t * size);
