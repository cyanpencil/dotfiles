...
void *slab_alloc(struct slab_allocator *slabs)
{
    ...
    size_t count_free = 0;
    struct slab_head *sh = NULL;
    for (struct slab_head *shh = slabs->slabs; shh != NULL; shh = shh->next) {
        if (shh->free != 0 && !sh) sh = shh;
        count_free += shh->free;
        if (count_free >= SLAB_KEEP) break;
    }

    if (count_free < SLAB_KEEP && !slabs->is_refilling) {
...
static errval_t slab_refill_pages(struct slab_allocator *slabs, size_t bytes)
{
    struct capref frame;
    size_t rsize;
    errval_t err = frame_alloc(&frame, BASE_PAGE_SIZE, &rsize);
    DBGERR(err, "frame_alloc");

    // FIXME do not use default paging_state
    void *slab_pglist_space_addr;
    err = paging_map_frame_attr(get_current_paging_state(), &slab_pglist_space_addr, 
                        rsize, frame, VREGION_FLAGS_READ_WRITE, NULL, NULL);
    DBGERR(err, "paging_map");

    slab_grow(slabs, slab_pglist_space_addr, rsize);
