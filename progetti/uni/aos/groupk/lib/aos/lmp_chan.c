...
errval_t lmp_chan_alloc_recv_slot(struct lmp_chan *lc)
{
    ...
    // Avoid nesting rpc calls (by not calling aos_get_ram_cap)
    errval_t err = two_level_alloc_no_refill(get_default_slot_allocator(), &slot);
    if (err_is_fail(err)) {
        return err_push(err, LIB_ERR_SLOT_ALLOC);
    }
    ...
    if (get_default_slot_allocator()->space < 5) {
        struct capref frame;
        slot_alloc(&frame);
    }
