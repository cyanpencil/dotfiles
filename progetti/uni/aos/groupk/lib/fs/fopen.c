...
static errval_t fs_open(void* st, const char* path, void *rethandle){ return mmchsfs_open(mount, path, rethandle); }

static errval_t fs_mkdir(const char *path){ return mmchsfs_mkdir(mount, path);}
static errval_t fs_rmdir(const char *path){ return mmchsfs_rmdir(mount, path); }
static errval_t fs_rm(const char *path){ return mmchsfs_remove(mount, path); }
static errval_t fs_opendir(const char *path, fs_dirhandle_t *h) {
    return mmchsfs_opendir(mount, path, h);
}
static errval_t fs_readdir(fs_dirhandle_t h, char **name) { return mmchsfs_dir_read_next(mount, h, name, NULL); }
static errval_t fs_closedir(fs_dirhandle_t h) { return mmchsfs_closedir(mount, h); }
static errval_t fs_fstat(fs_dirhandle_t h, struct fs_fileinfo *b) { return mmchsfs_stat(mount, h, b); }
...
static int fs_libc_open(char *path, int flags)
{
    ...
    // If O_CREAT was given, we use mmchsfsfs_create()
    if(flags & O_CREAT) {
        // If O_EXCL was also given, we check whether we can open() first
        if(flags & O_EXCL) {
            err = fs_open(mount, path, &vh);
            if(err_is_ok(err)) {
                mmchsfs_close(mount, vh);
                errno = EEXIST;
                return -1;
            }
        ...
        err = mmchsfs_create(mount, path, &vh);
...
static int fs_libc_read(int fd, void *buf, size_t len)
{
    ...
    {
        mmchsfs_handle_t fh = e->handle;
        assert(e->handle);
        err = mmchsfs_read(mount, fh, buf, len, &retlen);
        if (err_is_fail(err)) {
            return -1;
        }
    }
...
static int fs_libc_write(int fd, void *buf, size_t len)
{
    ...
    {
        mmchsfs_handle_t fh = e->handle;
        errval_t err = mmchsfs_write(mount, fh, buf, len, &retlen);
        if (err_is_fail(err)) {
            return -1;
        }
    }
...
static off_t fs_libc_lseek(int fd, off_t offset, int whence)
{
    ...
    case FDTAB_TYPE_FILE:
    {
        ...
        err = mmchsfs_seek(mount, fh, fs_whence, offset);
        if(err_is_fail(err)) {
            DEBUG_ERR(err, "vfs_seek");
            return -1;
        }

        err = mmchsfs_tell(mount, fh, &retpos);
        if(err_is_fail(err)) {
            return -1;
        }
        return retpos;
...
/*
 * These are wrap functions that call either mmchsfs 
 * or ramfs appropriately
 */
