...
gensize_t page_round_up(gensize_t size){
    if          (IN_RANGE(size, 1, 4*KB)) {
        ...
        return 1*MB;
    } else if   (IN_RANGE(size, 1*MB + 1, 16*MB)) {
        return 16*MB;
    } else if   (IN_RANGE(size, 16*MB + 1, 64*MB)) {
        return 64*MB;
    } else {
...
enum bucket_size bucket_type(gensize_t size) {
    if          (IN_RANGE(size, 1, 4*KB)) {
        ...
        return BucketSize_16M;
    } else if   (IN_RANGE(size, 16*MB + 1, 64*MB)) {
        return BucketSize_64M;
    } else {
                 ...
                 void *slot_alloc_inst)
{
        ...
        mm->slabs.refill_func = slab_refill_func ? slab_refill_func : slab_default_refill;

        dlist_init(&mm->masters_dlist);
        dlist_init(&mm->head_dlist);
        for (uint8_t i = 0; i < BUCKETS_NO; ++i) {
            dlist_init(&mm->free_buckets[i]);
        }
...
errval_t mm_add(struct mm *mm, struct capref cap, genpaddr_t base, size_t size)
{
    ...
    dlist_tail_insert(&mm->masters_dlist, master_node);
...
struct mmnode* mm_bucket_find(struct mm *mm, size_t size, size_t alignment, struct capref *retcap) {
    struct mmnode* node = (struct mmnode*) dlist_head(&mm->free_buckets[bucket_type(size)]);
    while(node) {
        /**
...
errval_t mm_free(struct mm *mm, struct capref cap, genpaddr_t base, gensize_t size)
{
    ...
    dlist_remove(&mm->head_dlist, (dlinked_node_t*) node);
    node->type = NodeType_Free;

    // Insert the free node in the according bucket
    dlist_head_insert(&mm->free_buckets[node->bucket], node);
