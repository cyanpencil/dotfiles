...
def draw_graph():
    ...
    points = (pf_input, predict_output(feature_matrix, weights))
    # ax.plot(pf_input, predict_output(feature_matrix, weights), "k--", lw=2)
    # pf_input.sort()
    # feature_matrix[:,1].sort()
    # ax.plot(points, "k-")
...
def gradient_descent_multiple(weights, feature_matrix, output):
    ...
    for t in range(150):
        ...
        for k, w in enumerate(weights):
            ...
            # for i, x_i in enumerate(feature_matrix):
                # prediction = np.dot(weights, feature_matrix[i]).sum()
                # for j,we in enumerate(weights):
                    # prediction += we*feature_matrix[i][j]
                # gradient += err[i] * feature_matrix[i][k]
...
n = 4
feature_matrix = np.zeros(len(pf_input) * 3)
feature_matrix.shape = (len(pf_input), 3)
X = np.array(pf_input)
feature_matrix[:,0] = 1
feature_matrix[:,1] = X
...
gradient_descent_multiple(weights, feature_matrix, pf_output)
draw_graph()
