...
# for c in vf.columns:
    # if vf[c].dtype != object and rf[c].std() != 0:
        # vf[c] = (vf[c] - rf[c].mean()) / rf[c].std()
...
def add_features(feature_list, df):
    ...
    # for e,c in enumerate(df.columns):
        # if e < 959 and c != str_output and df[c].dtype != object:
            # feature_list += [np.array(df[c])]
            # # feature_list += [np.array(df[c])**2]
            # # feature_list += [np.array(df[c])**3]
...
print(weights)
print(get_RSS_matrix(weights, validation_matrix, vf[str_output].tolist()))
