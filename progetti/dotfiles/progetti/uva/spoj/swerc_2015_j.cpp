...
Triangoli:
    ...
    a/sin(alfa) = b/sin(beta) = c/sin(gamma) = 2*R (raggio circocentro)
Cerchi:
    equazione: (x - a)^2 + (y - b)^2 = r^2
    pi = acos(-1.0)
    lunghezza corda: sqrt(2 * r^2 * (1 - cos(theta))), oppure 2r*sin(theta/2)
 */
...
//IMPORTANTE: to accept collinear points, we have to change the "> 0"
...
bool inConvexPolygon(point p, const vector<point> &P) {
    point z = P[0];
    int lo = 1, hi = P.size() - 2;
    while (lo < hi) {
        int m = (lo + hi) / 2;
        if (!ccw(z, P[m], p)) hi = m - 1;
        else lo = m + 1;
    }
    printf("Il punto (%f, %f) sta nella diagonale che va dal punto (%f, %f) al punto (%f, %f)\n", p.x, p.y, z.x, z.y, P[lo].x, P[lo].y);
}
...
int main() {
    ...
    vector<point> p = CH(large);
    double max_x = 0, max_y = 0, min_x = 99999999, min_y = 99999999, med_x = 0, med_y = 0;
    for (int i = 0; i < p.size() - 1; i++) {
        max_x = max(max_x, p[i].x);
        max_y = max(max_y, p[i].y);
        min_x = min(min_x, p[i].x);
        min_y = min(min_y, p[i].y);
        med_x += (p[i].x) / (p.size() - 1);
        med_y += (p[i].y) / (p.size() - 1);
    }
    point c = point(med_x, med_y), d;
    double min_distance = 99999999;
    for (int i = 0; i < p.size() - 1; i++) {
        min_distance = min(min_distance, distToLineSegment(c, p[i], p[i + 1], d));
    }
    int sol = 0;
    for (int i = 0; i < small.size(); i++) {
        if (small[i].x < min_x || small[i].x > max_x || small[i].y < min_y || small[i].y > max_y) {continue;}
        if (distPoints(small[i], c) < min_distance) {sol++; continue;}
        //if (inPolygon(small[i], p)) {
            //sol ++;
        //}
    }
    cout << sol << endl;
    ...
    //for (int i = 0; i < p.size(); i++) printf("(%f, %f)\n", p[i].x, p[i].y);
    //cout << inConvexPolygon(point(3, 3), p) << endl;
}
