...
//IMPORTANT! theta is in degrees
//ATTENZIONE! Non si capisce bene se ruota in senso orario o antiorario
    ...
    while(N--) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int i = 0; i < p.size(); i++) {
                point a = rotatePoint(p[i], - theta);
                polygon.push_back(point(a.x + x, a.y + y));
            }
            area_boards += w*h;
        }

        vector<point> P = CH(polygon);
        if (area(P) == 0) printf("100.0 %%\n");
        else printf("%.1f \%\n", area_boards / area(P) * 100.0);

        //for (int i = 0; i < P.size(); i++) printf("(%f, %f)", P[i].x, P[i].y);
    }
}
