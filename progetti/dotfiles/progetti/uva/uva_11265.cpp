...
void printPolygon(vector<point> &P) {
...
int main() {
    int N, W, H, x, y, caso = 0;
    while (scanf(" %d %d %d %d %d", &N, &W, &H, &x, &y) == 5) {
        caso++;
        vector< vector<point> > square[2];
        vector<point> q;
        q.push_back(point(0,0));
        q.push_back(point(W,0));
        q.push_back(point(W,H));
        q.push_back(point(0,H));
        q.push_back(point(0,0));
        square[0].push_back(q);
        int used = 0;
        for (int i = 0; i < N; i++) {
            square[(i + 1)%2].clear();
            int x1, y1, x2, y2;
            cin >> x1 >> y1 >> x2 >> y2;
            //cout << "Faccio il taglio " << i << ", numero poligoni: " << square[i%2].size() << endl;
            for (int j = 0; j < square[i%2].size(); j++) {
                vector<point> pol = cutPolygon(point(x1, y1), point(x2, y2), square[i%2][j]);
                vector<point> pol2 = cutPolygon(point(x2, y2), point(x1, y1), square[i%2][j]);
                //printPolygon(pol);
                //printPolygon(pol2);
                if (pol.size() > 0 && inPolygon(point(x, y), pol)) square[(i+1) % 2].push_back(pol);
                if (pol2.size() > 0 && inPolygon(point(x, y), pol2)) square[(i+1) % 2].push_back(pol2);
            }
        }
        for (int i = 0; i < square[N%2].size(); i++) {
            if (inPolygon(point(x, y), square[N%2][i])) {
                printf("Case #%d: %.3f\n", caso, area(square[N%2][i]));
            }
        }
    }
}
